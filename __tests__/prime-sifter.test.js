import { numberArray, primeSift } from "../src/js/prime-sifter";
...
describe('primeSift', () => {
    ...
    test('Should call itself with the returned array, and filter for numbers that result in 0 for i % 2', () => {
        expect(primeSift([2, 3, 5, 7, 9, 11, 13])).toEqual([2, 3, 5, 7, 11, 13]);
    });

    test('Should terminate when it reaches itself', () => {
        expect(primeSift(13)).toEqual([2, 3, 5, 7, 11, 13]);
    });
});